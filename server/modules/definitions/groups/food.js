...
for (let tier = 0; tier < 6; tier++) {
  for (let poly of ["egg", "square", "triangle", "pentagon", "hexagon"]) {
    for (let shiny of [
      ...
      Class[`laby${tier}${food[0].toUpperCase() + food.slice(1)}`] = makeLaby(
        Class[food],
        tier,
      );
    }
    Class[`laby${tier}${poly[0].toUpperCase() + poly.slice(1)}Crasher`] =
      makeCrasher(Class[`laby${tier}${poly[0].toUpperCase() + poly.slice(1)}`]);
  }
}
